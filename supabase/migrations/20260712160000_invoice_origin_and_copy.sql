alter table public.invoices
  add column if not exists invoice_origin text not null default 'standard'
  check (invoice_origin in ('standard', 'recurring_generated'));

alter table public.invoices
  add column if not exists send_copy boolean not null default true;
...
begin
  ...
  select timezone, payment_terms_days, invoice_notes, send_copy
    into v_timezone, v_payment_terms_days, v_invoice_notes, v_send_copy
