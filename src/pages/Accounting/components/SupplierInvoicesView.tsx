...
type SupplierInvoicesViewProps = {
  ...
  onNewInvoice: () => void;
  onSetPaid: (invoice: SupplierInvoiceWithDetails, paid: boolean, date: string, bankAccountId: string | null) => Promise<void>;
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.5fr)_minmax(300px,0.7fr)] lg:items-start">
      ...
      <Panel>
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="text-right"><p className="text-2xl font-semibold">{formatCurrency(Number(invoice.total))}</p><Tag tone={status.tone}>{status.label}</Tag></div>
        </div>
        <dl className="mt-5 grid gap-4 border-y border-blue-100 py-4 text-sm sm:grid-cols-4">
        ...
        <Panel>
          ...
          {invoice.status === "cancelled" ? <p className="mt-2 text-sm text-slate-500">Fakturaen er annullert.</p> : (
            <>
              ...
              <Button className="mt-4 w-full" variant={invoice.status === "paid" ? "secondary" : "success"} disabled={busy} onClick={() => invoice.status === "paid" ? setShowCorrection(true) : void onSetPaid(invoice, true, paymentDate, bankAccountId || null)}>{busy ? "Bokfører..." : invoice.status === "paid" ? "Korriger betaling" : "Registrer som betalt"}</Button>
            </>
          )}
        </Panel>
        <Panel>
