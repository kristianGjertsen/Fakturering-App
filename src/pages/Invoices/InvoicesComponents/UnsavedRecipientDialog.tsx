...
type UnsavedRecipientDialogProps = {
  open: boolean;
  onCancel: () => void;
...
export function UnsavedRecipientDialog({
  open,
  onCancel,
  onCreateCompany,
  onContinue,
}: UnsavedRecipientDialogProps) {
  if (!open) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/40 p-4" role="presentation">
      <section
        ...
        role="dialog"
        aria-modal="true"
        aria-labelledby="unsaved-recipient-title"
      >
        <h2 id="unsaved-recipient-title" className="text-lg font-semibold text-slate-950">
